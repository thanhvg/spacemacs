...
keybindings are almost the same as =edebug-mode-map=
#+begin_src elisp
(defvar-keymap edebug-mode-map
  :parent emacs-lisp-mode-map
  ;; control
  "SPC"     #'edebug-step-mode                            ;; change to S
  "n"       #'edebug-next-mode                            ;; change to N
  "g"       #'edebug-go-mode
  "G"       #'edebug-Go-nonstop-mode 				              ;; behind ,a
  "t"       #'edebug-trace-mode
  ...
  "h"       #'edebug-goto-here                             ;; change to H
...
=?= while edebug mode to check the keybindings, if they are reported as shawdowed, then they can be found in =,a= or global =c-x X=
