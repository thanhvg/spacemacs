...
(defconst spacemacs-bootstrap-packages
  '(
    ...
    (pcre2el :step pre)
    (holy-mode :location (recipe :fetcher local) :step pre :toggle 'nil)
    (hybrid-mode :location (recipe :fetcher local) :step pre :toggle 'nil)
...
(defun spacemacs-bootstrap/init-evil ()
 ...
 [_m_] all        [_r_] all"
  ...
  (define-key evil-normal-state-map "g/" 'spacemacs/duplicate-current-line-or-region)
